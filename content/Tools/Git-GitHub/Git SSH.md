...
SSH 除了作为远程 shell 之外，还可以使用远程的 Git 服务。
比如使用 Github, 可以使用命令 `ssh -T git@github.com`.  用 git 用户身份连接远程 Github。git 事 Github 上设置的一个特殊的用户 user。
这个命令的目的就是用于测试本机能够通过 SSH 密钥向 GitHub 完成身份认证。

>① `-T` 表示不分配 PTY(伪终端)。
  ② 客户端请求的是 exec channel, 即，执行一条远程命令。
  ③ GitHub 服务器为 git 这个用户配置了一个特殊的 shell(通常是 git-shell). 它只允许执行几个特定的命令，如 `git-upload-pack`, `git-receive-pack` 等。
  ④ 当 `git push` 执行推送时，本地 git 命令通过 SSH 在远端执行 `git-receive-pack`, 两端通过stdin/stdout 在这个 channel 上对话和传输数据。
...
我们首先在本地生成一个 SSH Key. 然后将生成的公钥 public key 复制到 Github上面去，在 Github 的 settings 页面上完成注册。这样在 Github仓库，在本机连接的时候，就能识别本机了。完成身份验证之后，就可以使用 git 命令完成各种远程仓库的操作了。具体配置方式如下：
...
就不需要 SSH key，但 push 时通常要用 GitHub 登录或 Personal Access Token。长期使用更推荐 SSH，一次配置好后比较省心。
...
其次，验证身份时，它传送的不是私钥 Key 本身, 而是一次私钥签名。这个过程跟密码登录完全不同，大致流程如下：
...
所以，ssh 首先找到私钥，然后用它对应的公钥去登录服务器，服务器认证后，返回一个随机数，ssh 在本地用私钥对随机数签名，再发给服务器，服务器验证成功后，连接成功建立。

<u>**私钥从头到尾没离开你的电脑**</u>，只有"用私钥算出来的签名结果"被发出去了。这就是非对称加密的核心安全性——攻击者就算截获了全部通信，也拿不到私钥，也伪造不出新的签名。
...
执行 `ssh -T git@github.com` 这个命令就是测试本机能否通过 SSH 密钥向 Github 完成身份认证。如果身份认证成功，那么会看到输出

![](Pasted%20image%2020260926110459.png)
...
`git push` 执行时也是同一套机制。
它默认绑定的远程仓库的地址 origin，比如是 `git@github.com:YourName/tech-notes.git` 
这个 URL 里：
