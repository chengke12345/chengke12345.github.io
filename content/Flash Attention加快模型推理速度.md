「✅」
我们训练好一个语言模型以后，使用它来做推理，这个过程叫做 [inference]。

#简述语言模型内部工作原理 
语言模型就是文字接龙，给一个句子，它去预测接下来产生哪一个 [token]。今天主流类神经网络的架构就是[Transformer]. Transformer 里面有很多层的Layer，每个 Layer中都有一个机制，[self-attention]。让Tranformer可以考虑整个输入sequence的所有信息。
![[Pasted image 20260324132940.png]]
self-attention, 输入一排向量 x1~x5, 输出一排向量 o1 ~ o5.
self-attention,首先把 x1~x5 各自乘以三个不同的矩阵，每个得到三个向量，q，k，v。 q是query, k 是 key, v是value。这个过程叫做，从 x 向量变成 Q K V 的过程。接下来以 x4 为例
![[Pasted image 20260324134205.png]]
把 x4 的 query, q4 和它前面所有的 key, k1 ~ k4，做 dot product，内积， 记为 a1 ~ a4。他们值可能是负无穷到正无穷，所以对他们做softmax操作, 进行normalization, 让它变成数值介于 0～1 之间的 $\hat{a_1}...\hat{a_4}$ ，它们的和为1。
然后$\hat{a_1}...\hat{a_4}$ 各自乘上 v 向量，做 weighted sum，最后得到 attention layer 在第4个位置的输出o4。
...
#GPU运算底层逻辑
在 GPU 里面，有一堆 Execution Unit, 它们负责做计算。这个 Execution Unit，在 GPU里面有很多个，不止一个，它们可以同时执行任务。我们把它比作[小精灵]
它们的优点是数量多，所以他们的运算非常快速。它们的弱点是工作台太小了，它们的工作台能存放的数据是有限的。如果有一个大矩阵或者大向量，我们没法把它摆上工作台，每次工作台上都只能放非常少量的数据来进行计算。这个工作台就是 [S-RAM]，或者更精确的说，是 [on-chip 的 SRAM]。我们称为[工作台]
如果GPU有大量数据，这些数据会被放在一个仓库里面，叫做 HBM (显存)。仓库大小是有限的，但是相较于工作台，仓库非常大，可以放非常多的内容。
...
搬数据，是拖慢运算的瓶颈。
Flash Attention 的思想就是，能不能改变 self-attention 计算的方法，结果一样，只是改变一些计算顺序，减少搬运数据的次数，这样让整体运算更快一点。
...
我们不能一次把所有的 key 都放到工作台上，因为这些 key 太多了。key的数量就是我们要处理的 sequence 的长度。如果LLM当做 AI Agent 来使用，它的输入往往非常长。 我们把一整排 key 的长度假设为 L， 代表现在输入的 sequence 的长度。我们要把这一整排key, 切成一个一个的[chunk]， 假设每个chunk有N个key。chunk的大小取决于工作台的大小。一个chunk中的N个key，以及 query 被放到工作台上，然后一瞬间，就完成了query 和 所有 key 的 dot product。算完后，再把这些 dot product 的结果数值，放回到仓库里面， 工作台清除空间。
![[Pasted image 20260324145818.png\|342]] ![[Pasted image 20260324145859.png\|342]]
然后是第二个 chunk 的 key, 送到工作台与 q 做 dot product。然后是第三个chunk的key, 以此类推。一次可以把一个chunk里面，所有key跟query的dot product计算出来。 
输出的每一个黑色小框，我们称为 $a_i$, 它是一个 attention 的 weight ($q_i\times k_i$)
我们真正想要的，是接下来做过 normalization 以后的attention weight, 记为$\hat{a_i}$.
![[Pasted image 20260324151712.png]]
nomalize的计算，是把每一个 $a_i$ 取 exponential 作为分子，把所有 $a_i$ 的exponential 加起来作为分母。得到 nomalize 的 $\hat{a_i}$。

这是理论上的计算方法，实际上我们还会在每一个 $a_i$ 上减去这一排 $a_i$ 里面最大的一个 dot product(weight) 值 $a_{max}$ 。这样 e 的指数项最大数值是 0，因为 $a_i$ 可以很大，如果它很大，计算 $e^{a_i}$ 的时候，在 Exponential 指数项就会放大成一个很大的数值，计算完就可能overflow。所以用 $a_i$ 减掉 $a_{max}$，确保指数项最大的数值就是0. 这是为了防止计算overflow。
![[Pasted image 20260324152809.png]]
在把 $a_i$ normalize 转化为 $\hat{a_i}$ 时，我们不可以把所有 $a_i$ 一次放入工作台，然后完成计算以后，又一次搬回仓库。这样是不行的。<font color="orange">工作台上不能放任何跟sequence长度 L 有关的东西，虽然这个 a 是scalar，一个一个的标量，但是实际上 L 可能是10万，100万。就算是简单数值，也无法放在工作台上</font>。

首先，要找出 $a_{max}$, 我们在工作台上只能看到一个chunk 的 a, 所以
![[Pasted image 20260324153135.png]]
我们一次，先 Load 一个chunk大小里面的a, 找出第一个chunk里面的最大 a, 假设最大的数值时$d_1$, 把这个数值存在工作台上的 d 空间处。d 中存放的就是当前这个chunk 里最大的 a。
然后把 a 中第二个chunk 读入工作台，比较这些 a 与 d 谁大，找出最大值 $d_2$ 放入 d 空间。
![[Pasted image 20260324153831.png]]
总共要做 B = L / N 次。做完之后，d 中存放的值就是 $a_{max}$
然后，我们对 a 分 chunk 得到分子 $a^{a_i-d}$，称为 $a_i$'。
得到分子之后，我们要算分母。按 chunk 求和就行了。
...
<font color="orange">在整个过程中，我们对仓库做了很多次读写操作</font>

#FlashAttention简化版 #softmax简化版
上面self-attention的计算方式，多次读写仓库。Flash Attention的思想就是要减少这样对仓库的读写次数。
从上面的过程中，我们可以看出，从 $a_i$ 到 $\hat{a_i}$ 要读好几次仓库。
$a_{max}$ 与 分母 $\sum_{i=1}^{L}e^{a_i-a_{max}}$ 可以一次被找出来。
![[Pasted image 20260324161253.png]]
我们先算第一个chunk, chunk 里的最大 a 为 $d_1$ , 假设它就是 $a_{max}$。我们得到一个分母, 求和式为 $s = s_1 = \sum_{i=1}^{N}e^{a_i-d_1}$
...
<font color="#eb4349">这就是 Flash Attention 的一个核心技巧。</font>
接下来的 chunk 都是相同的操作
...
#FlashAttention完整版 #softmax的完整实现
...
前面操作没有变化。q 要和一个chunk里面的 每个 k 做 dot product, 得到 $a_i$, 得到一个最大的设为 $d_1$放在 d 中。同时，用 $d_1$ 算出一个分母 $S_1$ 放在 S 中。
...
这就是Flash Attention的精神，我们真正要得到的是这个o。这个过程反复进行下去。所以，最终在最后一个 B chunk以后，我们就得到。
